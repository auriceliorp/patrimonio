...
//	Modulo: Alteracao de local (Comissao de inventario)
...
//$Texto = "Membro da Comissão de Inventário,<BR>";
//$Texto = $Texto . "Este módulo permite alterar a localização dos bens patrimoniais além de identificar o membro executor.<BR>O procedimento se encontra no final do formulário.<BR>";
$Texto = "Este módulo permite alterar a localização dos bens patrimoniais além de identificar o membro executor.<BR>O procedimento se encontra no final do formulário.<BR>";
...
$Titulo = "Alteração de local - Comissão de Inventário";
mc_dados( $Titulo );
...
//
	...
	if ( $_POST[resposta] == 'N' ) {
		...
		echo "<FORM ACTION='./bem-local-comissao.php' METHOD='post'>";
	...
	$texto = addslashes("[" . troca_data($DataHoje) . "] - Troca de local efetuada por membro da Comissão de Inventário.<BR>Membro: $_SESSION[nome]");
	...
	if ( $rEquip = mysql_query( $qEquip ) ) {
		...
		$qModifica = "INSERT INTO bemtbmodificacao
				...
				VALUES ('', '', '$bem[0]', '$bem[1]', '$bem[2]', '$_POST[local]', '$DataHoje', '$DataHoje', '0', '0', '$texto')";
