...
//		}
		...
		//
		$msg = mail(	"$usernovo[2]@ctaa.embrapa.br",
...
echo "<TD><SELECT NAME='mat'>";
	while ( $DadosFunc = mysql_fetch_array( $rFunc ) ) {
		if ( $DadosFunc[Matricula] == $umResp ) {
			echo "<OPTION SELECTED VALUE= '$DadosFunc[Matricula]#$DadosFunc[Nome]#$DadosFunc[Username]'>$DadosFunc[Nome]</OPTION>";
		} else {
			echo "<OPTION VALUE= '$DadosFunc[Matricula]#$DadosFunc[Nome]#$DadosFunc[Username]'>$DadosFunc[Nome]</OPTION>";
		}
	}
echo "</SELECT></TD>";
echo "</TR>";
